<template>
  ...
  >
    ...
    <ElsText v-if="title" v-bind="textAttr" :is-hover="isHover" />
    <template v-for="({ link, ...c }, i) in contents" :key="i">
      <div
        :class="['_link-item cursor-pointer relative', contentClass]"
        @click="() => handleLink(link)"
      >
        <ElsContent v-bind="mergeProps(common, c)" :is-hover="isHover" />
      </div>
    </template>
  </div>
</template>
